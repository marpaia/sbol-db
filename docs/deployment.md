...
| `extraEnv` | `SBOL_DB_PUBLIC_ORIGIN` | Bound listener origin | Externally reachable HTTP(S) origin advertised to CLI and MCP clients. Required behind an ingress or reverse proxy; do not include a path. |
| `extraEnv` | `SBOL_DB_MCP_ENABLED` | `true` | Mount authenticated MCP at `/mcp` and advertise it from `/api/v2/instance`. |
