...
export default function SparqlRoute() {
  const buffer = useWorkbenchStore((s) => s.buffers.sparql);
  const setBuffer = useWorkbenchStore((s) => s.setBuffer);
  const pushHistory = useWorkbenchStore((s) => s.pushHistory);
  ...
  const insertIntoEditor = (text: string) => {
    const existing = useWorkbenchStore.getState().buffers.sparql;
    const next =
