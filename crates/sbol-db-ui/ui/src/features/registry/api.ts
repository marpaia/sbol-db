import {
  ...
  requestEmpty as requestEmptyResponse,
  requestJson as requestJsonResponse,
} from "@/shared/api/http";

export class PortalApiError extends HttpError {
...
async function requestJson<T>(
  ...
  return requestJsonResponse<T>(input, init, responseError);
}
...
async function requestEmpty(
  ...
  return requestEmptyResponse(input, init, responseError);
}
...
async function responseError(response: Response): Promise<PortalApiError> {
  ...
  const parsed = parseStructuredErrorBody(body);
  message = parsed?.message ?? message;
  return new PortalApiError(response.status, message, parsed?.code);
}
