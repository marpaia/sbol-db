//! Durable SBOL Identity OAuth grants over SQLite.
...
impl OAuthStore for SqliteOAuthStore {
    async fn register_client(&self, client: OAuthClient) -> Result<(), DomainError> {
        sqlx::query(
            "INSERT INTO sbol_oauth_client \
             (client_id, client_name, redirect_uris, created_at) VALUES (?, ?, ?, ?)",
        )
    ...
    async fn get_client(&self, client_id: &str) -> Result<Option<OAuthClient>, DomainError> {
        let row = sqlx::query(
            "SELECT client_id, client_name, redirect_uris, created_at \
             FROM sbol_oauth_client WHERE client_id = ?",
        )
        .bind(client_id)
        ...
        sqlx::query(
            "INSERT INTO sbol_oauth_authorization_code \
             (code_hash, user_id, client_id, redirect_uri, resource, scopes, \
              code_challenge, nonce, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(code.code_hash)
        .bind(code.user_id.as_uuid().to_string())
        .bind(code.client_id)
    ...
    async fn consume_authorization_code(
        ...
        let row = sqlx::query(
            "DELETE FROM sbol_oauth_authorization_code WHERE code_hash = ? \
    ...
    async fn issue_access_token(&self, token: OAuthAccessToken) -> Result<(), DomainError> {
        ...
        .bind(token.user_id.as_uuid().to_string())
    ...
    async fn resolve_access_token(
        ...
        let row = sqlx::query(
            "SELECT token_hash, user_id, client_id, resource, scopes, expires_at, created_at \
             FROM sbol_oauth_access_token WHERE token_hash = ?",
        )
        .bind(token_hash)
        .fetch_optional(&self.pool)
        .await
    ...
    async fn revoke_access_token(&self, token_hash: &str) -> Result<bool, DomainError> {
        let result = sqlx::query("DELETE FROM sbol_oauth_access_token WHERE token_hash = ?")
            .bind(token_hash)
            .execute(&self.pool)
            .await
            .map_err(db_err)?;
    ...
    async fn consume_refresh_token(
        ...
        let row = sqlx::query(
            "DELETE FROM sbol_oauth_refresh_token WHERE token_hash = ? \
...
fn client_from_row(row: SqliteRow) -> Result<OAuthClient, DomainError> {
    let redirect_uris: String = row.try_get("redirect_uris").map_err(db_err)?;
    Ok(OAuthClient {
...
fn code_from_row(row: SqliteRow) -> Result<OAuthAuthorizationCode, DomainError> {
...
fn access_from_row(row: SqliteRow) -> Result<OAuthAccessToken, DomainError> {
    Ok(OAuthAccessToken {
...
fn refresh_from_row(row: SqliteRow) -> Result<OAuthRefreshToken, DomainError> {
...
fn user_id(row: &SqliteRow) -> Result<UserId, DomainError> {
    let value: String = row.try_get("user_id").map_err(db_err)?;
    Ok(UserId(uuid::Uuid::parse_str(&value).map_err(db_err)?))
}

fn scopes(row: &SqliteRow) -> Result<Vec<String>, DomainError> {
    let value: String = row.try_get("scopes").map_err(db_err)?;
