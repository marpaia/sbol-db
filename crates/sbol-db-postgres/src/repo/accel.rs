...
impl AccelRepository {
    ...
    /// Every occurrence of one resource, ordered by graph IRI. The graph-local
    /// accelerator metadata is the universal RDF-derived resource catalog.
    pub async fn resource_occurrences(
        &self,
        iri: &str,
    ) -> Result<Vec<ResourceOccurrence>, DomainError> {
        let rows = sqlx::query(
            "SELECT graph_iri, meta FROM accel_object WHERE iri = $1 ORDER BY graph_iri",
        )
        .bind(iri)
        .fetch_all(&self.pool)
        .await
    ...
    /// Globally keyset-page unique resource IRIs, applying filters before the
    /// limit. Only the returned page is expanded to occurrence metadata.
    pub async fn resources(
        ...
        if let Some(text) = query.text.as_deref().filter(|text| !text.is_empty()) {
            ...
            qb.push(" AND (o.iri ILIKE ")
                .push_bind(needle.clone())
                .push(" OR o.meta ILIKE ")
                .push_bind(needle)
                .push(")");
        }
    ...
    /// Expand a page of resource identities in one query. Keeping this separate
    /// from the keyset query avoids one database round trip per list item.
    ...
    pub async fn sequence(&self, iri: &str) -> Result<Option<CatalogSequenceRecord>, DomainError> {
        let graph_count: i64 = sqlx::query_scalar(
            "SELECT count(DISTINCT graph_iri) FROM accel_type WHERE iri = $1 \
        ...
        let rows = sqlx::query(
            r#"
            SELECT predicate_iri::text AS predicate_iri,
                   object_iri::text AS object_iri, object_literal
            FROM sbol_triples
            WHERE subject_iri = $1
            ...
            ORDER BY graph_iri NULLS LAST, id
            "#,
        )
        .bind(iri)
        .fetch_all(&self.pool)
    ...
    /// Expand sequence counts and values with two set queries, independent of
    /// page size. The first value in graph/id order remains the representative.
    async fn sequences_for_iris(
        ...
        let mut value_qb: QueryBuilder<sqlx::Postgres> = QueryBuilder::new(
            "SELECT subject_iri::text AS subject_iri, predicate_iri::text AS predicate_iri, \
                    object_iri::text AS object_iri, object_literal \
        ...
        value_qb.push(") ORDER BY subject_iri, graph_iri NULLS LAST, id");
